...
public class Problem58 {
	...
	public static void main(String[] args) {
		long totalNumbers = 1 , totalPrimes = 0 , number = 1 , adder = 0;

        // starting with 1 I go +2 +2 +2 +2 ,
        // then (starting with 9) + 4 + 4 + 4 + 4 ,
        // then + 6 + 6 + 6 + 6 ...
        // the trick here is to notice that pattern, after that its simple
        do  {
            adder += 2;
            for(int i = 0; i < 4; i++) {
                number += adder;
        ...
        } while(((double) totalPrimes / (double)totalNumbers) >= 0.1);

        // +1 here because sideLength = adder + 1
        System.out.println(adder + 1);
    }
}
